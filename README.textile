...
A webkit alapú böngészők (Safari és Google Chrome) autocomplete funkciója bekavar a plugin működésébe, a label tag nem tűnik el. A probléma elkerülhető a lentebb leírt módszerrel. A hiba megkerülése folyamatban van.

h2. Használat

h3. HTML

A HTML kódban érdemes használni az autocomplete="off" taget a webkit hiba elkerülése érdekében.

<pre>
<p>
  <label for="valami_id">Label Text</label>
  <input type="text" name="field_id" id="valami_id" value="" autocomplete="off" />
</p>
</pre>
...
<pre>
/* csak azokra a p elemekre, melyekben szerepel label HTML tag! */
p {
	position: relative;
}
